...
public class DirectMappingTest {
	...
	public static Collection<Object[]> getTestLists() {
		...
		for (File f:dir) {
			if (f.isDirectory() && new File(f.getAbsolutePath()+ "/directGraph.ttl").exists()) {
				absolutePath = f.getAbsolutePath();
				arr = new Object[3];
				arr[0] = "jdbc:hsqldb:mem:" + f.getName();
				arr[1] = absolutePath + "/create.sql";
				arr[2] = absolutePath + "/directGraph.ttl";
				lstTests.add(arr);
			}
		}
		return lstTests;
	...
	public void directMappingValidator() {
		...
		Model model = getJenaModel(this.dGraphFilePath);
